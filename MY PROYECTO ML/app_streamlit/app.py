...
def get_signal(xau_df, dxy_df, pipe, feat_cols, tl, ts, atr_min, adx_min, dxy_p, atr_hi):
    ...
    try:
        ...
        df_fe=build_features(xau_df.tail(400),dxy_df.tail(400) if dxy_df is not None else None)
        fa=[f for f in feat_cols if f in df_fe.columns]
        df_fe=df_fe.dropna(subset=fa)
        if len(df_fe)>0:
            proba=float(pipe.predict_proba(df_fe[fa].iloc[[-1]].values)[0,1])
    except: pass
...
# ── Señal ─────────────────────────────────────────────────────────────
if art:
    pipe=art["pipeline"]; fc=art["feature_cols"]
...
if art:
    try:
        ...
        fa=[f for f in art["feature_cols"] if f in df_fe.columns]
        df_fe=df_fe.dropna(subset=fa)
        pr=art["pipeline"].predict_proba(df_fe[fa].values)[:,1]
        rows=[{"Fecha":dt.strftime("%Y-%m-%d %H:%M"),
               "P(BUY)":f"{p:.3f}",
               "Señal ML":"🟢 BUY" if p>tl else ("🔴 SELL" if p<ts else "⚪ HOLD")}
              for dt,p in zip(df_fe.index[-20:],pr[-20:])]
...
st.markdown("---")
st.markdown("<div style='text-align:center;color:#4B5563;font-size:0.82em'>⚠️ Solo apoyo a decisiones · <b style='color:#F59E0B'>Print Money Factory</b> · Alexsandro · Madrid 2026</div>",unsafe_allow_html=True)
