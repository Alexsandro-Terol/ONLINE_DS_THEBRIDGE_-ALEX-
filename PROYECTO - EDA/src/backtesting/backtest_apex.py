"""
╔══════════════════════════════════════════════════════════════════╗
║          APEX BACKTESTER — Sistema completo EDA-based           ║
║   Activos : XAUUSD · XAGUSD · DXY                               ║
║   Señales : Ratio Oro/Plata + DXY filter + Rolling correlation  ║
║   Capital  : $500                                               ║
╚══════════════════════════════════════════════════════════════════╝
"""
...
SL_MULTIPLIER     = 1.5        # stop = 1.5 × ATR
TP_MULTIPLIER     = 2.5        # take profit = 2.5 × ATR  → ratio 1:2.5
...
def generate_signals(df):
    ...
    for i in range(1, len(df)):
        ...
        # SEÑAL A: Long Plata / Short Oro
        if (row['GSR_zscore'] > 1.0 and
            row['DXY_mom3'] < -0.15 and
            row['roll_corr_oro'] < -0.2):
            sig.iloc[i]        = 1
            trade_type.iloc[i] = 'A_LongPlata_ShortOro'

        # SEÑAL B: Long Oro / Short Plata
        elif (row['GSR_zscore'] < -0.8 and
              row['DXY_mom3'] > 0.15 and
              row['roll_corr_oro'] < -0.2):
            sig.iloc[i]        = 2
            trade_type.iloc[i] = 'B_LongOro_ShortPlata'

        # SEÑAL C: Long Oro puro
        elif (row['DXY_mom3'] < -0.3 and
              row['roll_corr_oro'] < -0.1 and
              -0.5 < row['GSR_zscore'] < 0.5):
            sig.iloc[i]        = 3
...
for t in ['A_LongPlata_ShortOro', 'B_LongOro_ShortPlata', 'C_LongOro']:
...
for i in range(len(df)):
    ...
    if not in_trade and signal > 0:
        ...
        hit_sl = (stype != 'B_LongOro_ShortPlata' and current_price <= sl) or \
                 (stype == 'B_LongOro_ShortPlata' and current_price >= sl)
        hit_tp = (stype != 'B_LongOro_ShortPlata' and current_price >= tp) or \
                 (stype == 'B_LongOro_ShortPlata' and current_price <= tp)

        # Máximo 10 días en trade (time exit)
        days_in = (date - trade_open['entry_date']).days

        if hit_sl or hit_tp or days_in >= 10:
            if hit_tp:
...
fig.suptitle('APEX BACKTESTER — Sistema EDA-based | XAUUSD · XAGUSD · DXY | 2016–2026',
